...
const deleteStudentSubmission = (submissionId: number) => {
    if (confirm('Are you sure you want to delete this student submission? This action cannot be undone.')) {
        router.delete(route('submissions.destroy', submissionId), {
            preserveScroll: true,
            onSuccess: () => {
                // Success notification can go here
            }
        });
    }
};
        ...
        <Teleport to="body">
            <div class="dynamic-breadcrumbs fixed top-2 z-[9999] flex items-center h-16 pointer-events-none">
                <nav class="flex items-center text-[16px] font-normal pointer-events-auto">
                    <Link :href="route('dashboard')" class="text-black transition-colors duration-200 hover:text-[#FFD700] tracking-tight">Dashboard</Link>
                    <ChevronRight class="mx-3 h-3 w-3 text-black stroke-[2px]" />
                    
                    <template v-if="!isOwnProfile">
                        <Link :href="route('students.index')" class="text-black transition-colors duration-200 hover:text-[#FFD700] tracking-tight">Students</Link>
                        <ChevronRight class="mx-3 h-3 w-3 text-black stroke-[2px]" />
                        <span class="text-[#FFD700] font-medium tracking-tight">{{ student.name }}</span>
                    </template>
                    <template v-else>
                        <span class="text-[#FFD700] font-medium tracking-tight">My Progress</span>
                    </template>
                </nav>
            </div>
        </Teleport>
                                    ...
                                    <td v-if="!isOwnProfile" class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <button 
                                                ...
                                                class="rounded p-1 text-gray-400 hover:bg-red-900/50 hover:text-red-400 transition-colors"
...
<style>
.dynamic-breadcrumbs { left: 305px; transition: left 0.3s ease-in-out; }
body:has([data-state="collapsed"]) .dynamic-breadcrumbs, body:has(aside[data-state="collapsed"]) .dynamic-breadcrumbs { left: 115px !important; }
</style>